function randRenyiSparse(nNodes, dw, T)
    runBBP   = false;
    runMooij = true;
    ...
    errors   = cell(1, nTrials);
        
    fn = sprintf('/var/tmp/kt2384/ZeroER/mk_renyi_dw_%d_T_%d_nNodes_%d', dw, T, nNodes);        
    ...
    for t = 1:nTrials
        ...
        maxDeg     = max(sum(adj, 1));

        problems{t} = var2struct(theta, W, maxDeg);
